...
LAB3D-SDL2-GLES is a port of Ken's Labyrinth to modern operating systems, using
OpenGL-ES for graphics output and the SDL2 library to provide user input, sound
output, threading, and some graphics support functions. Music output is
through Adlib emulation.

- Runs natively on 32-bit and/or 64-bit Unix/Linux.
- Supports big-endian CPUs.
- Uses OpenGL-ES to provide hardware accelerated graphics with
  trilinear interpolation in true colour (if available).
